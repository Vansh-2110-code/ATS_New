...
import {
  ...
  Edit3, RefreshCw, PlusCircle, Tag, Bookmark, Sparkles,
} from 'lucide-react';
...
export function ResumeListPage({ lockedStatus }: { lockedStatus?: string }) {
  ...
  const [tagJrCand, setTagJrCand] = useState<any>(null);
    ...
    const fetchCandidates = async () => {
      try {
        ...
        setCandidates(list.map((c: any) => ({
          ...
          jrNumber: c.jrNumber || '',
          originalJrNumber: c.originalJrNumber || '',
          originalJobTitle: c.originalJobTitle || '',
          originalClientName: c.originalClientName || '',
          originalScreenedAt: c.originalScreenedAt || null,
          tlRejectedAt: c.tlRejectedAt || null,
          tlRejectionReason: c.tlRejectionReason || '',
  ...
  const filtered = candidates.filter(c => {
    ...
    const matchSearch =
      ...
      (c.jrNumber && c.jrNumber.toLowerCase().includes(q)) ||
      (c.originalJrNumber && c.originalJrNumber.toLowerCase().includes(q)) ||
  ...
  const handleTagJrSubmit = async () => {
    if (!tagJrCand || !selectedJrNumber) return;
    try {
      setTaggingJr(true);
      setTagJrError('');
      await api.tagCandidateToNewJr(tagJrCand.id, {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-5">
                        ...
                        <div>
                          ...
                          {c.originalJrNumber && (
                            <div className="flex items-center gap-1 my-0.5">
                              <span
                                className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 bg-amber-50 text-amber-800 border border-amber-200 rounded font-medium"
                                title={`Original JR: ${c.originalJrNumber} (${c.originalJobTitle || 'Screened Eligible'}) - Available for re-assignment without duplicate screening`}
                              >
                                <span className="text-amber-600">🏷️</span>
                                <span className="font-semibold">{c.originalJrNumber}</span>
                                <span className="text-amber-600 font-normal">· Screened Eligible</span>
                              </span>
                            </div>
                          )}
                    ...
                    <td className="px-5 py-4 text-center sticky right-0 bg-white group-hover:bg-slate-50/90 z-10 shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)] border-l border-slate-50 transition-colors">
                      <div className="flex items-center justify-center gap-1.5 flex-wrap w-[220px]">
                        ...
                        <button
                          ...
                          style={{ fontWeight: 500 }}
                          title="Tag or Re-assign to New JR without repeating screening"
                        >
      ...
      {/* ── Tag Candidate to New JR Modal (Available for all Recruiters/TL/Admin) ── */}
      {tagJrModalOpen && tagJrCand && (
        <div className="fixed inset-0 bg-slate-950/45 flex items-center justify-center p-4 z-50">
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-xs mt-0.5">Fast-track screened eligible candidate to a new JR</p>
                </div>
              </div>
            ...
            <div className="p-6 space-y-4">
              <div className="bg-amber-50/70 border border-amber-200 rounded-xl p-3.5 text-xs text-amber-900 space-y-1.5">
                <div className="flex items-center justify-between">
                  <p className="font-semibold text-sm text-slate-800">{tagJrCand.name}</p>
                  <span className="text-[11px] px-2 py-0.5 bg-green-100 text-green-800 font-semibold rounded">
                    Screened: Eligible
                  </span>
                </div>
                {tagJrCand.originalJrNumber && (
                  <p className="text-amber-800 font-medium">
                    🏷️ Original JR: <strong>{tagJrCand.originalJrNumber}</strong> {tagJrCand.originalJobTitle ? `(${tagJrCand.originalJobTitle})` : ''}
                  </p>
                )}
                <p className="text-slate-600">
                  Current JR: <strong className="text-slate-700">{tagJrCand.jrNumber || 'None'}</strong> · Client: <strong className="text-slate-700">{tagJrCand.clientName || 'N/A'}</strong>
                </p>
                <div className="mt-2 pt-2 border-t border-amber-200/60 text-[11px] text-amber-800">
                  ✨ <strong>Audit Guarantee:</strong> Tagging to a new JR permanently preserves the candidate's original JR reference and screening history for tracking. No full re-screening is required!
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g., Fits Python requirements, previous eligible candidate fast-tracked from General Pool..."
